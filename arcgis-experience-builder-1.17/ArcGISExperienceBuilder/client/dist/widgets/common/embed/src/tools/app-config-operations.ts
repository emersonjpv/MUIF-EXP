import { Immutable, expressionUtils, dataSourceUtils, type Expression, type DuplicateContext, type extensionSpec, type IMAppConfig } from 'jimu-core'
...
export default class AppConfigOperation implements extensionSpec.AppConfigOperationsExtension {
  ...
  afterWidgetCopied(
    ...
    expression = (needToMapDs && needToMapDs?.length > 0) ? mapExpression(expression, contentMap) : expression
  ...
  useDataSourceWillChange (appConfig: IMAppConfig, dataSourceId: string, newDataSourceId?: string): IMAppConfig {
    ...
    if(dataSourceId || newDataSourceId) {
      ...
      const config: IMConfig = widgetJson?.config
      let expression = config.expression
      expression = replaceExpressionDataSource(expression, dataSourceId, newDataSourceId)
...
export const replaceExpressionDataSource = (html: string, dataSourceId: string, newDataSourceId?: string) => {
  if (!html) return html
  return html.replace(richTextUtils.EXP_TAG_REGEXP, (exp) => {
    ...
    return exp.replace(richTextUtils.DATA_EXPRESSION_REGEXP, (match, encoded) => {
      ...
      const expression = expressionUtils.replaceDataSourceId(Immutable(info), dataSourceId, newDataSourceId)
...
export const mapExpression = (html: string, contentMap: DuplicateContext) => {
  if (!html) return html
  return html.replace(richTextUtils.EXP_TAG_REGEXP, (exp) => {
