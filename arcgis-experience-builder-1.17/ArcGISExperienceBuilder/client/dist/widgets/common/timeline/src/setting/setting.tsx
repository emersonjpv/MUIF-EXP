...
class _Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps, State> {
  ...
  dataSourceChange = async ds => {
    ...
    if (ds.length < this.props.useDataSources?.length) {
      ...
      let honorTimeSettings = currentDsObj.type === DataSourceTypes.WebMap ? config.honorTimeSettings : false

      // ds type is changed, or replaced to another webMap
      if (currentDsObj.type !== config.dataSourceType || currentDsObj.type === DataSourceTypes.WebMap) {
        newDs = [currentDs]
  ...
  hideDs = (dsJson) => {
    ...
    } else if (ds.type === DataSourceTypes.WebMap) { // check all layers inside.
  ...
  setSourceMode = (addSourceByData) => {
    ...
    onSettingChange({
      ...
      config: config
        ...
        .set('honorTimeSettings', true)
        .set('dataSourceType', DataSourceTypes.WebMap)
        .set('timeSettings', null)
    })
  }
  ...
  getTimeSettingsContent = (addSourceByData) => {
    ...
    >
      {
        dataSourceType === DataSourceTypes.WebMap
          ? <React.Fragment>
            <SettingRow>
