    ...
    return searchSuggestion?.map((layerSuggestion, index) => {
      ...
      return (
        <div key={`${layerSuggestion?.layer}-${index}`} role='listbox' aria-label={layerSuggestion?.layer}>
          {isShowLayerName && <Button aria-label={layerSuggestion?.layer} role='button' className='source-label-con jimu-outline-inside' disabled={true} title={layerSuggestion?.layer}>
