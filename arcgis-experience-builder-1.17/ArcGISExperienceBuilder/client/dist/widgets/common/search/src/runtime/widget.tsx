/** @jsx jsx */
import { React, css, jsx, type AllWidgetProps, Immutable, type ImmutableArray, polished, type UseUtility, UtilityManager, hooks, moduleLoader, defaultMessages as jimUiDefaultMessage } from 'jimu-core'
import { ViewVisibilityContext, type ViewVisibilityContextProps, PageVisibilityContext } from 'jimu-layouts/layout-runtime'
import type * as jimuUi from 'jimu-ui/advanced/setting-components'
...
const Widget = (props: SearchProps) => {
  ...
  const getDefaultGeocodeDataConfig = async (customKeys?: string) => {
    const settingComponents = await moduleLoader.loadModule<typeof jimuUi>('jimu-ui/advanced/setting-components')
  ...
  */
  const initGeocodeList = (configItem: NewDatasourceConfigItem) => {
    ...
    const { countryCode, configId, hint, geocodeURL, outputDataSourceId, label, searchInCurrentMapExtent, enableLocalSearch, minScale, searchServiceType, spatialReference, singleLineFieldName, displayFields, defaultAddressFieldName, addressFields, isSupportSuggest, useUtility } = configItem
    const dataSourceListItem = serviceList?.[configId]?.asMutable({ deep: true }) || {}
    const updateItem = {
      hint: hint,
