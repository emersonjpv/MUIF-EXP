...
import { TextInput, Button, Link, Loading, defaultMessages as jimuiDefaultMessage } from 'jimu-ui'
  ...
  */
  const publishRecordCreateAction = () => {
    ...
    Promise.all([geocodeRecords]).then(res => {
      ...
      publishRecordCreatedMessageAction(id, allResponse, RecordSetChangeType.CreateUpdate, extent)
    })
  }
  ...
  */
    ...
    return (
      <div className='d-flex align-items-center'>
        {(searchValue && searchValue?.length > 0) &&
          <Button icon type='tertiary' size='sm' aria-label={nls('clear')} title={nls('clear')} onClick={() => { clearSearchValue() }}>
            <CloseOutlined/>
          </Button>}
      </div>
    )
  ...
  const renderLoading = (isShowLoading: boolean, locationLoading: boolean) => {
    ...
    return <div className={classNames('position-absolute loading-con', { 'hide-loading': !showLoading })}>
      <Loading className='w-100' type='BAR'/>
    </div>
  }
