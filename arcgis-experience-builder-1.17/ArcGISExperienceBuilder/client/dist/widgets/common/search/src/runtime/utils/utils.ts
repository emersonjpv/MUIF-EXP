...
import {
  RECENT_SEARCHES_KEY, SourceType, type IMConfig, type SearchDataConfig, type SuggestionItem, type ServiceList, type SearchStatus,
...
import type * as jimuUiSettingComponents from 'jimu-ui/advanced/setting-components'
...
*/
export const publishRecordCreatedMessageAction = (widgetId: string, recordsArr: RecordResultType[], recordSetChangeType: RecordSetChangeType = RecordSetChangeType.CreateUpdate, extent?: __esri.Extent) => {
...
export async function createDsByDefaultGeocodeService (defaultGeocodeConfig: NewDatasourceConfigItem[], widgetId: string) {
  const settingComponents = await moduleLoader.loadModule<typeof jimuUiSettingComponents>('jimu-ui/advanced/setting-components')
