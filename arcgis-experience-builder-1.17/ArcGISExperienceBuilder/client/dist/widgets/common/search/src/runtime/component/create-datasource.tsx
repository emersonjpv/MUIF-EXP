...
const CreateDatasource = (props: CreateDatasourceProps) => {
  const { config, id, serviceList, dsStatus, synchronizeSettings, jimuMapView, handleSearchResultChange, handleSelectionListChange, handleDsStatusChange } = props
  ...
  const renderDatasourceComponent = (serviceListItem: IMServiceListItem, configId: string, hadInputSearchText: boolean) => {
    ...
    if (serviceListItem.searchServiceType === SearchServiceType.FeatureService) {
      ...
      const query = getQueryByServiceListItem(serviceListItem?.asMutable({ deep: true }), jimuMapView, config.sourceType, serviceListItem.searchInCurrentMapExtent)
