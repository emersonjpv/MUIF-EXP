...
async function getBufferedUnionGeometry (
  ...
  if (rawGeometries.length > 0) {
    if (enableBuffer && bufferDistanceInputValue > 0 && bufferUnitSelectValue) {
      ...
      const bufferResult = await geometryEngineAsync.buffer(rawGeometries as __esri.GeometryUnion[], [bufferDistanceInputValue], apiBufferUnit, unionResults)
      ...
      if (rawGeometries.length === 1) {
        ...
        // union rawGeometries
        bufferedUnionGeometry = await geometryEngineAsync.union(rawGeometries as __esri.GeometryUnion[])
      }
    }
  }
