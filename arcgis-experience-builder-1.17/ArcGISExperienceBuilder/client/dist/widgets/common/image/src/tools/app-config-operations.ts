...
export default class AppConfigOperation implements extensionSpec.AppConfigOperationsExtension {
  ...
  useDataSourceWillChange (appConfig: IMAppConfig, oldDataSourceId: string, newDataSourceId: string): IMAppConfig {
    ...
    if (dataSourceUtils.areDerivedFromSameMain(oldDataSourceId, newDataSourceId)) {
      newFunctionConfig = functionConfig
        .set('srcExpression', expressionUtils.replaceDataSourceId(functionConfig.srcExpression, oldDataSourceId, newDataSourceId))
        .set('altTextExpression', expressionUtils.replaceDataSourceId(functionConfig.altTextExpression, oldDataSourceId, newDataSourceId))
  ...
  afterWidgetCopied (
    ...
    const { linkParam, isChanged } =  utils.mapLinkParam(contentMap, config.functionConfig.linkParam)
    if (isChanged) {
    ...
    const checkSrcExpressionResult = expressionUtils.mapExpression(contentMap, srcExpression)
    ...
    const checkAltExpressionResult = expressionUtils.mapExpression(contentMap, altTextExpression)
