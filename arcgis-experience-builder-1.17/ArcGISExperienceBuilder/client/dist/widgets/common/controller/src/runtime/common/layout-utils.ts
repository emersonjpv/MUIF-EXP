...
export const useIsSelected = (controllerId: string) => {
  ...
  const isSelected = ReactRedux.useSelector((state: IMState) => {
    const selection = state.appRuntimeInfo.selection
    const selectedWidgetId = state.appConfig.layouts[selection?.layoutId]?.content?.[selection?.layoutItemId]?.widgetId
    return controllerId === selectedWidgetId || controlledWidgetIds.includes(selectedWidgetId)
  })
  return isSelected
}
