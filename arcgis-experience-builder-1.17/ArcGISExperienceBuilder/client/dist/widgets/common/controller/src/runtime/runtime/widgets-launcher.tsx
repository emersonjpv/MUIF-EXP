/**@jsx jsx */
import {
  React, jsx, appActions, getAppStore, type Size, hooks, ReactRedux, type IMState,
...
import { getMoreButtonNode, getWidgetCardNode, toggleWidget, useRootVisible } from './utils'
...
export default function WidgetsLauncher (props: WidgetsLauncherProps) {
  ...
  const handleCloseWidget = React.useCallback((evt: React.MouseEvent<HTMLDivElement> | React.KeyboardEvent<HTMLDivElement>, widgetId: string) => {
    ...
    const widgetButtonNode = getWidgetCardNode(widgetId) || getMoreButtonNode(id)
  ...
  const rootVisible = useRootVisible(rootRef.current)
