...
const LayerModeSetting = (props: FeatureFormSettingProps) => {
  ...
  const handleLayerConfigChange = React.useCallback((newLayerConfig: ImmutableObject<LayersConfig>) => {
    ...
    const updateConfigUseDs = newLayerConfig.useDataSource
    newUseDataSources = newUseDataSources.map(useDs => {
      if (updateConfigUseDs.dataSourceId === useDs.dataSourceId) {
        return updateConfigUseDs
      }
      return useDs
    })
    // new add
    if (!activeLayerConfig) {
      const originIndex = newUseDataSources.findIndex(useDs => useDs.dataSourceId === newLayerConfig.useDataSource.dataSourceId)
      if (originIndex > -1) {
        const originUseDs = newUseDataSources[originIndex]
        const mergedUseDataSource = mergeOneUseDataSource(newLayerConfig.useDataSource, originUseDs)
        const mutableUseDss = newUseDataSources.asMutable({ deep: true })
        newUseDataSources = Immutable(mutableUseDss.splice(originIndex, 1, mergedUseDataSource))
      } else {
        newUseDataSources = newUseDataSources.concat(newLayerConfig.useDataSource)
      }
    } else if (activeLayerConfig?.id !== newLayerConfig.id) {
      newUseDataSources = newUseDataSources.filter(useDs => useDs.dataSourceId !== activeLayerConfig?.useDataSource?.dataSourceId)
      newUseDataSources = newUseDataSources.concat(newLayerConfig.useDataSource)
    }
    onSettingChange({
      id: widgetId,
  ...
  }, [activeIndex, activeLayerConfig, config, layersConfig.length, onSettingChange, useDataSources, widgetId])
  ...
  return <React.Fragment>
    {layersConfig.map(config =>
      <LayerConfigDataSource
        key={config.useDataSource.dataSourceId}
