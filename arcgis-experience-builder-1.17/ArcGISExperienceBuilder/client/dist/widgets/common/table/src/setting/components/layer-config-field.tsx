...
import { defaultMessages as jimuUIMessages, Checkbox, Button, Label, Select } from 'jimu-ui'
...
import { type TableFieldsSchema, LayerHonorModeType } from '../../config'
import defaultMessages from '../translations/default'
import { getDataSourceById, INVISIBLE_FIELD } from '../../utils'
...
const LayerConfigField = (props: LayerConfigFieldProps) => {
  ...
  const { enableEdit, useDataSource, searchFields, tableFields: imTableFields, layerHonorMode } = layerConfig
  ...
  const constructTreeItem = React.useCallback((tableFields: TableFieldsSchema[]) => {
    ...
    const showFieldsToTreeItem = (mutableGroupedFields: TableFieldsSchema[]) => {
      return mutableGroupedFields.map((item) => ({
  ...
  }, [enableEdit, theme, useDataSource?.dataSourceId, getFieldEditable, layerDefinition])
  ...
  const onFieldChange = React.useCallback((allSelectedFields: IMFieldSchema[]) => {
    ...
    onChange(layerConfig.set('searchFields', filteredSearchFields).set('tableFields', newTableFields).set('useDataSource', mewUseDs))
    constructTreeItem(newTableFields)
  }, [constructTreeItem, filterSearchFields, tableFields, layerConfig, layerDefinition?.fields, onChange])
