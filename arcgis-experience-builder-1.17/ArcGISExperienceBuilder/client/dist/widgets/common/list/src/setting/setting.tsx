...
> {
  ...
  onCardSettingReturnBackClick = evt => {
    ...
    setTimeout(() => {
      if (showCardSetting === Status.Hover) {
        focusElementInKeyboardMode(this.toHoverSettingButtonRef)
      }
      if (showCardSetting === Status.Selected) {
        focusElementInKeyboardMode(this.toSelectedSettingButtonRef)
      }
    }, 100)
  }
  ...
  componentDidUpdate (
    ...
    const { settingPanelChange } = this.props
    this.toggleMessageActionSettingOpenStateInExpressMode()
    this.updateConfigWhenDataSourceChange(preProps)
  ...
  updateConfigWhenDataSourceChange = (preProps) => {
    ...
    if (useDataSources !== preProps.useDataSources) {
      const oldDs = preProps.useDataSources?.[0]
      const currentDs = this.props.useDataSources?.[0]
  ...
  getTemplateSize = () => {
    ...
    const templateSize = {
      ...
      STYLE12: { width: this.checkTemplateDefaultSize(87000 / parentWidth), height: this.checkTemplateDefaultSize(33500 / parentHeight) }
    }
    return templateSize
  }
  ...
  getSelectModeOptions = (): React.JSX.Element[] => {
    return [
  ...
  onDataSelect = (currentSelectedDs: UseDataSource) => {
    ...
    if (
      ...
      const areDerivedFromSameMain = dataSourceUtils.areDerivedFromSameMain(datasource?.id, currentSelectedDs?.dataSourceId)
    ...
    return (
      <div ref={ref => { this.setTemplatesContain(ref) }}>
        <SettingSection title={this.formatMessage('chooseTemplateTip')}>
          ...
          <SettingRow flow='wrap' role='group' aria-label={this.formatMessage('flow')} label={this.formatMessage('flow')}>
            <div className='style-group w-100'>
              <Button
                ...
                type='tertiary'
                className='w-100'
                title={`${this.formatMessage('flow')} 1`}
              >
                <Icon
                  autoFlip
            ...
            <div className='style-group w-100'>
              <Button
                ...
                type='tertiary'
                className='w-100'
                title={`${this.formatMessage('flow')} 2`}
              >
                <Icon
                  autoFlip
            ...
            <div className='style-group w-100'>
              <Button
                ...
                type='tertiary'
                className='w-100'
                title={`${this.formatMessage('flow')} 3`}
              >
                <Icon
                  autoFlip
  ...
  renderDataSetting = () => {
    ...
    return (
      <SettingSection className={classNames(!this.state.datasource ? 'no-bottom-border' : '')}>
        ...
        {this.checkIsDsAutoRefreshSettingOpen(datasource) && <SettingRow tag='label' label={this.formatMessage('lastUpdateText')} role='group' aria-label={this.formatMessage('lastUpdateText')}>
          <Switch
      ...
      <SettingSection>
        ...
        >
          ...
          {config.pageStyle === PageStyle.Scroll && (
            <SettingRow tag='label' label={this.formatMessage('scrollBar')} aria-label={this.formatMessage('scrollBar')}>
          ...
          {config.pageStyle === PageStyle.Scroll && (
            <SettingRow tag='label' label={this.formatMessage('navigator')} aria-label={this.formatMessage('navigator')}>
              <div className='d-flex'>
                <Switch
  ...
  getBrowserIcons = (iconSize: number) => {
    ...
    const desktopLabel = this.formatMessage('appliedToLargeScreen')
    const padLabel = this.formatMessage('appliedToMediumScreen')
    const mobileLabel = this.formatMessage('appliedToSmallScreen')
    return (
    ...
    return (
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestClose={this.closeSettingCollapse}
        >
          <SettingRow className="mt-2" tag='label' label={this.formatMessage('search')} aria-label={this.formatMessage('search')}>
            <div className='d-flex'>
              <Switch
          ...
          <SettingRow tag='label' label={this.formatMessage('sort')} aria-label={this.formatMessage('sort')}>
            <div className='d-flex'>
              <Switch
          ...
          <SettingRow tag='label' label={this.formatMessage('filter')} aria-label={this.formatMessage('filter')}>
            <div className='d-flex'>
              <Switch
          ...
          <SettingRow tag='label' label={this.formatMessage('refresh')} aria-label={this.formatMessage('refresh')}>
            <Switch
          ...
          {config.cardConfigs[Status.Selected].selectionMode !==
            SelectionModeType.None && (
            <Fragment>
              <SettingRow tag='label' label={this.formatMessage('showSelection')} aria-label={this.formatMessage('showSelection')}>
                <Switch
              ...
              <SettingRow tag='label' label={this.formatMessage('clearSelection')} aria-label={this.formatMessage('clearSelection')}>
                <Switch
          ...
          <SettingRow tag='label' label={this.formatMessage('showRecordCount')} aria-label={this.formatMessage('showRecordCount')}>
            <Switch
