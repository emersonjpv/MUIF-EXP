...
import {
  ...
  Immutable, css, type IMThemeVariables, hooks, type FeatureLayerDataSource, type SubtypeSublayerDataSource,
  dataSourceUtils, SupportedServerTypes, type MapDataSource, ServiceManager, DataSourceManager, type DataSourceTypes
} from 'jimu-core'
...
const getTipsStyle = (theme: IMThemeVariables) => {
  return css`
    .edit-tips{
      font-style: italic;
      font-size: 12px;
...
const FeatureFormSetting = (props: FeatureFormSettingProps) => {
  ...
  const onDataSourceCreatedOrFailed = React.useCallback(async (dataSourceId: string, ds: SupportedDataSource) => {
    ...
    if (!ds) {
      ...
      const newLayerInfo = {
        ...
        isFromMap: !!(dataSource?.getRootDataSource() as MapDataSource)?.map
      }
      setLayersInfo(old => {
        const newLayersInfo = {}
  ...
  return <React.Fragment>
    ...
    <div className='mt-4' ref={sidePopperTrigger}>
      ...
      <SettingRow css={getTipsStyle(theme)}>
    ...
    >
      <LayerConfig
        ...
        isFromMap={activeLayerInfo?.isFromMap}
        filterDs={onFilterDs}
