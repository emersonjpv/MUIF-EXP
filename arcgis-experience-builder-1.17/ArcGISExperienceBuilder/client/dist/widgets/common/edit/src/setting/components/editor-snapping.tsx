/** @jsx jsx */
import { React, jsx, hooks, defaultMessages as jimuCoreMessages } from 'jimu-core'
...
const EditorSnapping = (props: EditorSnappingProps) => {
  ...
  const { selfSnapping, featureSnapping, defaultSelfEnabled, defaultFeatureEnabled, gridSnapping, defaultGridEnabled, defaultSnapLayers, snapSettingMode } = config
  ...
  const snapLayersButtonProps = React.useMemo(() => ({ disabled: jimuMapViews.length === 0 }), [jimuMapViews.length])
  ...
  >
      ...
      ? <React.Fragment>
        ...
        <SettingRow>
          <Label className='d-flex align-items-center'>
            ...
            />
            {translate('featureSnapping')}
          </Label>
        </SettingRow>
      </React.Fragment>
      : <React.Fragment>
        <SettingRow tag='label' label={translate('selfSnapping')}>
          <Switch
        ...
        <SettingRow tag='label' label={translate('featureSnapping')}>
          <Switch
            className='can-x-switch'
    ...
    <SettingRow
      flow='wrap'
      label={translate('chooseDefault')}
      className='select-option'
    >
      <AdvancedSelect
        size='sm'
        title={translate('chooseDefault')}
        fluid
        hideCheckAll={false}
        ...
        buttonProps={snapLayersButtonProps}
        onChange={onSnapLayersChange}
        aria-label={translate('chooseDefault')}
      />
    </SettingRow>
  </SettingSection>
}
