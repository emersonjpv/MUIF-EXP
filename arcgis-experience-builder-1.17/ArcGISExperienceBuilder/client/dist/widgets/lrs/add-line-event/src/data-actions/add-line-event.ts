...
import { AbstractDataAction, type DataRecordSet, MutableStoreManager, DataLevel, getAppStore, type DataSource, DataSourceStatus, type JSAPILayerMixin, type ImmutableObject, loadArcGISJSAPIModules, DataSourceTypes, type FeatureLayerDataSource } from 'jimu-core'
        ...
        .then(async (results) => {
          if (isDefined(results)) {
            ...
            await Promise.all(results.features.map(async (feature) => {
              const fromDate = feature.attributes.FROM_DATE
              const toDate = feature.attributes.TO_DATE
              if ((fromDate === rteInfo.fromDate) && (toDate === rteInfo.toDate)) {
