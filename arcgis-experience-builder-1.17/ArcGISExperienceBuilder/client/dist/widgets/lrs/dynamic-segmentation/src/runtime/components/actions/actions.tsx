...
export function Actions (props: ActionsProps) {
  const { intl, routeId, allowMerge, widgetId, jimuMapView, highlightColor, lrsLayers, networkDS, networkInfo, dynSegFeatureLayer, onSave, handleFieldCalculator, attributeSet } = props
  const { records, selectedRecordIds, selectedSldId, display, highlightLayer } = useDynSegRuntimeState()

  React.useEffect(() => {

    const fetchData = async () => {
      await getGraphic(selectedRecordIds)
    }
    fetchData()
