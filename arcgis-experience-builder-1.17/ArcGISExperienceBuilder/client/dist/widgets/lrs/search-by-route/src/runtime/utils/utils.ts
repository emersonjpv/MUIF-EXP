import type { IntlShape, FeatureLayerDataSource } from 'jimu-core'
...
export function getPopupTemplate (intl: IntlShape, record: __esri.Graphic, outputDS: FeatureLayerDataSource, networkItem: ImmutableObject<NetworkItem>, selectedMethod: string, measureType: string, config: IMConfig) {
  ...
  if (showAdditionalFields) {
    ...
    fieldsToDisplay = getNetworkFieldNamesForDisplayOrig(networkItem, record, selectedMethod)
  }
 ...
 * */
function getNetworkFieldNamesForDisplayOrig (networkItem: ImmutableObject<NetworkItem>, record: __esri.Graphic, selectedMethod: string): string[] {
