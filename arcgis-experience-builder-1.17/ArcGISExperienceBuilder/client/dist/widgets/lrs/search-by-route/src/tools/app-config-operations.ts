...
export default class AppConfigOperation implements extensionSpec.AppConfigOperationsExtension {
  ...
  afterWidgetCopied (
    ...
    if (!contentMap) { // no need to change widget linkage if it is not performed during a page copying
        ...
        config.networkItems?.forEach((networkItem, index) => {
          if (networkItem.outputLineDsId && contentMap[networkItem.outputLineDsId]) {
            newAppConfig = newAppConfig.setIn(['widgets', destWidgetId, 'config', 'networkItems', `${index}`, 'outputLineDsId'], contentMap[networkItem.outputLineDsId])
          }
          if (networkItem.outputPointDsId && contentMap[networkItem.outputPointDsId]) {
            newAppConfig = newAppConfig.setIn(['widgets', destWidgetId, 'config', 'networkItems', `${index}`, 'outputPointDsId'], contentMap[networkItem.outputPointDsId])
          }
        })
