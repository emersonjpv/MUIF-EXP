...
const Widget = (props: BaseMapProps) => {
  ...
  const basemapGalleryRef = React.useRef<__esri.BasemapGallery>()
  const updateBasemapGallery = (basemapGallery: __esri.BasemapGallery) => {
  ...
  const onActiveViewChange = async (activeView: JimuMapView, previousActiveViewId: string) => {
    ...
    if (!basemapGallery && !hasGalleryCache) {
    ...
    } else if (basemapGallery && activeView) { // for the map view change case after the basemap gallery initialed correctly
    ...
    } else if (!basemapGallery && basemapGalleryRef.current && hasGalleryCache) {
      ...
      if (basemapGalleryRef.current.view !== activeView?.view) {
        createBasemapGallery(activeView?.view)
      }
    }
  }
  ...
  hooks.useUpdateEffect(() => {
    ...
    if (!basemapGallery || (basemapTypeUsedWhenGetSource.current === basemapsType && themeModeWhenGetSource.current === themeMode)) {
      return
    }
    updateBasemapGalleryItems()
  }, [basemapGallery])
  ...
  const updateWidgetContainerParent = (element: HTMLDivElement) => {
    ...
    if (!basemapGallery) {
      ...
      if (cachedGalleryInstance) {
        updateBasemapGallery(cachedGalleryInstance)
        setLoading(false)
      }
    }
  }
