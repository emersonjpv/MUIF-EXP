...
function Widget(props: AllWidgetProps<IMConfig> & ExtraProps) {
  ...
  const viewerContainerId = `oi-viewer-container-${props.id}-${jimuUtils.getUUID()}`
  ...
  const getOiLayers = useCallback((layerList: Collection<Layer>) => {
    return layerList
  ...
  }, [selectedOiLayer, view, oiViewer, removeOiViewer])
  ...
  const renderLayerSelector = () => {
    return (
      <CalciteSelect
        label={'Oriented Imagery Layer Selector'}
  ...
  return (
    <div
      ...
      className='jimu-widget overflow-auto p-2'
      style={{ backgroundColor: props.theme.ref.palette.white }}
    >
