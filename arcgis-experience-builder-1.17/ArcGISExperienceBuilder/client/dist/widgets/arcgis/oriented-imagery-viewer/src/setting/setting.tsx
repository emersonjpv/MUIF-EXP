...
import { settingsInfo } from './constants'
...
function Setting(props: AllWidgetSettingProps<IMConfig>) {
  ...
  const getOiLayers = (layerList: Collection<Layer>) => {
    ...
    return (
      <>
        {toolsInfo.map((toolInfo) => {
          ...
          return (
            <>
              <SettingRow tag='label' label={toolLabel} key={toolInfo.name}>
  ...
  const renderToolsConfig = () => {
    ...
    return <SettingSection title={nls('configureToolsSectionTitle')}>{renderSettings(settingsInfo)}</SettingSection>
  }
