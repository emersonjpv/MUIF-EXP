...
import {
  ...
  MessageType,
  type FeatureLayerDataSource, type FeatureDataRecord
} from 'jimu-core'
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, State> {
   ...
   */
  getSelectedFeatureRecordsOnLoad = (allDsLayers) => {
  ...
  */
  ...
  clearMessageAction = () => {
    ...
    if (this.selectedRecord?.dataSource) {
    ...
    if (layerAnalysisInfo.analysisType === AnalysisTypeName.Closest) {
      ...
      if (features.length > 1) {
        ...
        if (this.state.aoiGeometries?.incidentGeometry?.type === firstFeatureGeometry.type) {
          closestFeature = features.filter((f: FeatureDataRecord) => geometryEngine.equals(f.feature.geometry as __esri.geometryGeometry, this.state.aoiGeometries.incidentGeometry as __esri.geometryGeometry))
        }
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      if (this.featuresByDsId.hasOwnProperty(dsId)) {
        ...
        if (searchByLocation && layerInfo.analysisInfo.returnIntersectedPolygons && ds.getGeometryType() === 'esriGeometryPolygon') {
          featureList = featureList.filter((eachRecord) => geometryEngine.intersect(this.state.aoiGeometries?.incidentGeometry as __esri.geometryGeometry, eachRecord.feature.geometry))
  ...
  getArea = (featureRecords: DataRecord[], geoms: Geometry, distanceUnits: string): number => {
    ...
    featureRecords.forEach(featureRecord => {
      ...
      let intersectGeom
      if (geoms) {
        intersectGeom = geometryEngine.intersect(selectedFeatureRecord.feature.geometry, geoms as __esri.geometryGeometry)
      } else {
   ...
   */
  downloadIndividualCsv = (index: number, dsId: string, analysisId: string, analysisType: string) => {
  ...
  initializeForm = (geometryType) => {
    // if feature form is already intialized then dont initalize it again
    ...
    // if page number is 2 and aoigeometry type is polygon then show form for bufferGeoemtry
    // else show form for inciendent geometry
    const geometry = geometryType === 'polygon' && this.state.currentPage === 2 ? this.state.aoiGeometries.bufferGeometry : this.state.aoiGeometries.incidentGeometry
  ...
  renderFormForSavingInputLocation = () => {
    ...
    const formHeaderTitle = css`
        border-left: ${currentPage === 2 ? '1px solid rgb(213, 213, 213)' : 'none'};
        `
    const formBody = css`
        flex: 1;
        overflow-y: auto;
      ...
      <React.Fragment>
        <div css={formContainer}>
          <div css={formHeader}>
            ...
            <span css={formHeaderTitle}>
              <div css={formTitle} className='m-2 pt-1'>{formHeaderText}</div>
            </span>
          </div>
    ...
    this.analysisLayersResults.forEach((result) => {
      ...
      if (result.allowExport) {
        if (result.layerInfo.analysisInfo.analysisType === AnalysisTypeName.Summary) {
          ...
          const fieldsToExport = this.getFieldsToExport(result.layerInfo.analysisInfo.analysisId)
          result.records.forEach((eachRecord, index) => {
            const eachRow = []
            const fields = eachRecord.feature.layer.fields
            for (const attrKey in eachRecord.feature.attributes) {
  ...
  render () {
    ...
    return (
      <div ref={this.widgetConRef} css={getStyle(this.props.theme, this.state.listMaxHeight, this.state.generalSettings.noResultMsgStyleSettings, this.state.generalSettings.promptTextMsgStyleSettings, this.state.searchSettings?.headingLabelStyle, this.state.showFeatureForm)} className={'jimu-widget'}>
          ...
          <div className='main-row w-100 h-100'>
            ...
            <div className={'layerContainer'}>
              ...
              {!this.state.isAnalysisLayerConfigured && this.state.isLayerAvailable &&
                <Alert tabIndex={0} withIcon={true} size='small' type='info' className='w-100 shadow mb-1 m-0'>
