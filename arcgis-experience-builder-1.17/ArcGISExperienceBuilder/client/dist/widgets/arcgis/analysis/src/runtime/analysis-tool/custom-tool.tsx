...
import { type AnalysisToolDataItem, type AnalysisToolData, AnalysisEngine, AnalysisType, type LocaleItem, type AnalysisServiceInfo, type AnalysisToolValidationParams, AnalysisToolParamDataType, type AnalysisToolUI } from '@arcgis/analysis-ui-schema'
...
const CustomTool = (props: ToolProps) => {
  ...
  const parameters = useRef<AllParameters>()
  ...
  // eslint-disable-next-line no-undef
  const panelElementRef = useRef<HTMLCalcitePanelElement>()
    ...
    <div className='analysis-exb-custom-web-tool-container h-100 d-flex'>
      <CalcitePanel
        ...
        ref={(ref) => {
          ...
          if (!disableBack && !backButtonFocused && ref) {
            focusElementInKeyboardMode(ref)
            setBackButtonFocused(true)
          }
        }}
      >
        {!disableBack && <calcite-action
          class='analysis-panel-back-button'
