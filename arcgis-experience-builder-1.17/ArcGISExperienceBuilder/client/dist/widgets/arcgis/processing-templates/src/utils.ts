...
export const getActionDataSets = async (layer: __esri.Layer, jimuMapView: JimuMapView): Promise<DataRecordSet[]> => {
  ...
  const layerDataSource = mapDataSource.getDataSourceByLayer(layer) ?? await mapDataSource.createDataSourceByLayer(layer)
  const dataSets = [{ dataSource: layerDataSource, records: [], name: layerDataSource?.getLabel() }]
  return dataSets
}
