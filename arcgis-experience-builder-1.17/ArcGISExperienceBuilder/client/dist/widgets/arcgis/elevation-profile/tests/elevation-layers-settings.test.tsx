import { DataSourceManager, React, createIntl } from 'jimu-core'
...
const groundLayerInfo = [{
  dataSourceId: 'default',
  groundElevationLayerUrl: '',
  isGroundElevationLayerExists: true
}]

const elevationLayerSettingsProps = {
  widgetId: 'widget_2',
  groundLayerInfo: groundLayerInfo,
  availableStats: [],
  theme: mockTheme,
...
describe('Validate the elevation layer settings', function () {
  ...
  it('By default volumetric object option should be unchecked', function () {
    const ref: { current: HTMLElement } = { current: null }
    const WidgetSettings = wrapWidget(ElevationLayersSettings as any, { theme: mockTheme, ref } as any)
    const { getByTestId } = render(<WidgetSettings widgetId='elevationLayersSettings' {...elevationLayerSettingsProps} />)
    expect((getByTestId('volumetricObject') as any).checked).toEqual(false)
  })
