/** @jsx jsx */
import { jsx, React, css, polished, lodash, type ImmutableArray, Immutable, DataSourceTypes, type DataSourceJson, classNames, type UseUtility, SupportedUtilityType, loadArcGISJSAPIModules, hooks, getAppStore, UtilityManager, AllDataSourceTypes, DataSourceManager, type IMDataSourceJson } from 'jimu-core'
import { defaultMessages as jimuUIMessages, Select, Switch, Option } from 'jimu-ui'
  ...
  const onSearchDataSettingsChange = (settings: ImmutableArray<SearchDataConfig>) => {
    if (!lodash.isDeepEqual(settings, searchDataConfig)) {
      onSettingChange({
        ...
        useUtilities: getUsedUtilities(routeConfig?.useUtility, settings?.filter(c => !!c.useUtility).map(c => c.useUtility)?.asMutable())
      })
    }
  }
        ...
        hasMap
          ? <div>
            ...
            <SettingSection role='group' aria-label={translate('generalSettings')} title={translate('generalSettings')} className='general-settings'>
              ...
              <SettingRow tag='label' label={translate('showRuntimeLayers')}>
                <Switch
