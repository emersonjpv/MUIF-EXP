  ...
  const applyBufferEffect = React.useCallback(async () => {
    ...
    if (geometry.spatialReference?.isGeographic && !geometry.spatialReference.isWGS84) {
      ...
      if (geometry.spatialReference?.isWGS84 || geometry.spatialReference?.isWebMercator) {
        bufferedGraphicRef.current.geometry = geometryEngineRef.current.geodesicBuffer(
          geometry as __esri.geometryGeometry,
          bufferDistanceRef.current,
          lodash.kebabCase(bufferUnitRef.current) as any
        ) as __esri.Polygon
      } else {
        bufferedGraphicRef.current.geometry = geometryEngineRef.current.buffer(
          geometry as __esri.geometryGeometry,
          bufferDistanceRef.current,
          lodash.kebabCase(bufferUnitRef.current) as any
        ) as __esri.Polygon
