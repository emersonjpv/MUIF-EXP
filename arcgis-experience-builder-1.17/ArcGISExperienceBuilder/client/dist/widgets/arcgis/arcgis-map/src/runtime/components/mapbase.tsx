...
import {
  ...
  serviceUrlUtils, observeStore, type ResourceSessions, ServiceManager, SessionManager, type ArcGISSubLayer, type MapDataSource, DataSourceTypes
} from 'jimu-core'
import type { Unsubscribe } from 'redux'
import { type IMConfig, SceneQualityMode } from '../../config'
import { loadArcGISJSAPIModules, MapViewManager, type JimuMapView, zoomToUtils, type DefaultMapInfo, type ZoomToOptions } from 'jimu-arcgis'
...
import {
  ...
  type UrlParamsMapOptions, getMapOptionsByUrlHashParams, getChangedViewpointByViewAndUrlHashParams, layerQueryExtent, getDefaultHighlightOptions
} from '../utils'
...
import { type PopupDockPosition, getValidPopupDockPosition, getFinalScaleRangeForView } from '../../utils'
...
export default class MapBase extends React.PureComponent<Props, State> {
  ...
  applyFinalMinMaxScaleFor2dMapView (mapView: __esri.MapView): boolean {
    ...
    if (mapView.type === '2d') {
      ...
      const configScaleRange = this.props?.baseWidgetProps?.config?.scaleRange
      ...
      if (constraints) {
        ...
        if (finalScaleRange) {
          ...
          if (constraints.minScale !== finalMinScale) {
            ...
            // console.log(`mapView update minScale to ${constraints.minScale}`)
          }

          if (constraints.maxScale !== finalMaxScale) {
            scaleChanged = true
            constraints.maxScale = finalMaxScale
            // console.log(`mapView update maxScale to ${constraints.maxScale}`)
          }
        }
      }
    }
  ...
  debounceUpdateUrlHashParams = () => null
  ...
  async getMapViewOptionToCreateMapView (dataSourceInfo: MapbaseDataSourceInfo): Promise<__esri.MapViewProperties> {
    ...
    const config = this.props?.baseWidgetProps?.config
    mapViewOption.highlights = this.createInitialHighlights(config, this.Color)
  ...
  async getSceneViewOptionToCreateSceneView (dataSourceInfo: MapbaseDataSourceInfo): Promise<__esri.SceneViewProperties> {
    ...
    mapViewOption.highlights = this.createInitialHighlights(widgetConfig, this.Color)
  ...
  private createInitialHighlights(config: IMConfig, Color: typeof __esri.Color): __esri.HighlightOptions[] {
    const highlightOption = this.createDefaultHighlightOptionsByConfig(config, Color)
    const highlightOptions: __esri.HighlightOptions[] = [highlightOption]
      ...
      if (zoomToFeatureInternalValue.type === 'zoom-to-extent') {
        ...
        if (extentFunc) {
          ...
          if (extent?.declaredClass === 'esri.geometry.Extent') {
            ...
            zoomToUtils.zoomTo(mapBaseView, extent, zoomToOption)
          }
        }
