...
import { type IMConfig, SceneQualityMode, type ScaleRange } from '../config'
...
import { type PopupDockPosition, type CustomPopupDockPosition, getValidPopupDockPosition, getMinScaleAndMaxScaleByLods } from '../utils'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps, State> {
  ...
  getStyle () {
    ...
    const rtlCssOfLodRangeIndicatorScale = isRTL ? 'margin-right: 2px; transform: scale(-1);' : 'margin-left: 2px;'
    ...
    return css`
      ...
      .lod-range-indicator {
        /*
        When there is no scroll bar, 195px is fine, but when there is a vertical scroll bar,
        other widths should be used, and the width of the scroll bar is not fixed,
        so the width should be calculated dynamically.
        */
      ...
      .exb-scale-range-slider-setting-row {
        ...
        .exb-scale-range-slider-container {
          /*
          When there is no scroll bar, 216px is fine, but when there is a vertical scroll bar,
          other widths should be used, and the width of the scroll bar is not fixed,
          so the width should be calculated dynamically.
          */
  ...
  render () {
    ...
    const isScaleRangeSwitchEnabled = hasWebMapDataSource
    const isScaleRangeSwitchChecked = isScaleRangeSwitchEnabled && !!scaleRange
                  ...
                  <React.Fragment>
                    {
                      this.state.webMapInfos?.map(webMapInfo => {
                        return (
                          <SettingRow key={webMapInfo.jimuMapViewId}>
                            <DataMapOutlined
                              className='mr-4 scale-range-left-icon'
                              title={webMapInfo.mapName}
                            />
                            <LodRangeIndicator
                ...
                <SettingRow className='exb-scale-range-slider-setting-row'>
                  {
                    ...
                    <WarningOutlined
                      className='scale-range-left-icon mr-2'
            ...
            <div className='w-100'>
              {Object.keys(ToolModules).map((key, index) => {
                if (ToolModules[key].isNeedSetting) {
                  ...
                  const className = index === 0 ? 'mt-0' : 'mt-2'
