...
class SearchInner extends React.PureComponent<SearchInnerProps, SearchInnerState> {
  ...
  componentDidMount () {
    if (!this.state.apiLoaded) {
      loadArcGISJSAPIModules(['esri/widgets/Search', 'esri/portal/Portal', 'esri/core/reactiveUtils']).then(modules => {
        [this.Search, this.Portal, this.reactiveUtils] = modules
        this.setState({
          apiLoaded: true
        })
      })
  ...
  componentDidUpdate () {
    if (this.state.apiLoaded && this.container) {
      ...
      if (mapView) {
        this.popupEnabledWatchHandle = this.reactiveUtils.watch(() => mapView?.popupEnabled, () => {
